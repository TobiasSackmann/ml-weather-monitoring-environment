...
with DAG(
    ...
    read_data_task = PythonOperator(
        task_id="read_data_from_influxdb", python_callable=read_from_influxdb
    )

    # Task 2: Preprocess Data
    preprocess_data_task = PythonOperator(
        task_id="preprocess_data",
        python_callable=lambda ti: preprocess_dataframe(
            ti.xcom_pull(task_ids="read_data_from_influxdb")
        ),
    )
    ...
    send_to_tf_task = PythonOperator(
        ...
        python_callable=lambda ti: send_to_tf_serving(
    ...
    write_data_task = PythonOperator(
        ...
        python_callable=lambda ti: write_to_influxdb(
            ti.xcom_pull(task_ids="send_data_to_tensorflow")
        ),
    )
